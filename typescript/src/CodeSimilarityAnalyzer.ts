...
export class CodeSimilarityAnalyzer {
  private readonly structuralKeywords = new Set([
   ...
   */
  private extractStructuralFeatures(line: string): Set<string> {
   ...
   */
  private calculateSequenceSimilarity(seqA: string[], seqB: string[]): number {
   ...
   */
  private calculateLineSimilarity(lineA: string, lineB: string): number {
    ...
    if (stringSimilarity > 0.7) {  // High string similarity (likely variable name changes)
      ...
      similarity = (
        0.40 * stringSimilarity +       // High weight for similar structure
      ...
      similarity = (
        0.35 * sequenceSimilarity +     // Code structure and order
   ...
   */
  private isTrivialLine(normalizedLine: string): boolean {
   ...
   */
  private findSimilarLines(linesA: string[], linesB: string[], threshold: number = 0.7): SimilarMatch[] {
    ...
    const potentialMatches: { i: number; j: number; score: number }[] = [];
    for (let i = 0; i < linesA.length; i++) {
      for (let j = 0; j < linesB.length; j++) {
   ...
   */
  private interpretSimilarity(percentage: number, avgScore: number): string {
    if (percentage >= 90) {
   ...
   */
  public printDetailedReport(results: AnalysisResults): void {
    if (results.error) {
